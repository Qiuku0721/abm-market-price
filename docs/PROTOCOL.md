...
> ⚠️ 注：本协议原服务于「安卓 App 全自动采集」方案（**已弃用移除**）。
> 当前电脑端采集直接写入 SQLite（不经此 HTTP 上报）；仅网页查询接口（`/api/*`、`GET /export/*`）仍沿用。
...
- 非 2xx 响应统一为 `{ "error": { "code": "<code>", "message": "<说明>" } }`。
  > 实现注：PC 端（FastAPI）校验类错误返回其标准 `{ "detail": "..." }`；App 仅依据状态码分类
  > （400/406/422 不可重试、429/5xx 重试），不解析错误体，故二者可共存。
